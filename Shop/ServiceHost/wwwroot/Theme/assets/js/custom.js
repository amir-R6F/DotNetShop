...
function addToCart(id, name, price, picture) {
    ...
    if (CurrentProduct !== undefined) {
        products.find(x => x.id === id).count = parseInt(count) + parseInt(CurrentProduct.count);
    } else {
        let product = {
            id,
            name,
...
function updateCart() {
    ...
    products.forEach(x => {
            ...
            `<div class="single-cart-item">
                 ...
                 <div class="image">
                     <a href="single-product.html">
                         <img src="/productCategories/${x.picture}"
                             class="img-fluid" alt="">
                    </a>
                </div>
                <div class="content">
        ...
        $("#cart_items_wrapper").append(product);
    })
}
...
function ChangeCartCount(id, total_id, count){
    let products = $.cookie(cookieName);
    products = JSON.parse(products);
    let i = products.findIndex(x => x.id == id);
    products[i].count = count;
    let newTotal = parseInt(count) * parseInt(products[i].unitPrice);
    $(`#${total_id}`).text(newTotal);
    $.cookie(cookieName, JSON.stringify(products), {expires: 2, path: "/"});
    updateCart();
}
